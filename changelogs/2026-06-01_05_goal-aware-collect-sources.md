...
- `src/openreply/research/intents.py`: added `COLLECT_PROFILES` + `collect_profile()` and a `collect` field on every intent (surfaced via `list_intents()`/`get_intent()`). The `thesis` goal pins `sources=arxiv,openalex,pubmed,scholar`, `skip_reddit=True`, `aggressive=False` (`~3 min`); other goals stay `None` (unchanged full-sweep behaviour).
...
- `src/openreply/research/intents.py`, `app-tauri/index.html`, `app-tauri/src/main.js`, `app-tauri/src/screens/collect.js`, `app-tauri/src/style.css`
