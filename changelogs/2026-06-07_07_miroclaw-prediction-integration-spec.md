...
in OpenReply or its features should be re-implemented. Finding: miroclaw is *downstream*
of OpenReply (it consumes `openreply search`/`audience_personas`/`find_gaps`), so it cannot
...
- Mapped overlap against existing OpenReply code: `persona/*`, `research/deliberate.py`,
  `research/audience.py`, `research/strategy_common.py`, `corpus_temporal_split`.
- Confirmed OpenReply already satisfies the LLM seam the porting guide requires.
...
- `docs/specs/MIROCLAW_OPENREPLY_FULL_ANALYSIS.md` — exhaustive master analysis: full
  ...
  miroclaw's 12 finance data sources vs OpenReply's ~30 (verdict: 3 redundant, 3 irrelevant,
