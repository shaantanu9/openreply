...
1. **Zero MCP coverage** — Claude Code could `gapmap_search` (Reddit API), `gapmap_query_db` (raw SQL), and `reddit_graph_*` (graph traversal), but couldn't do **semantic search** over the 12,108-post local corpus. The 79 MB ONNX model was a sunk cost the MCP never used.
...
- **5 new MCP tools** (file: `src/reddit_research/mcp/server.py`):
  - `gapmap_palace_status` → `{installed, ready, count, archive_bytes, expected_bytes, cache_dir, palace_dir}`
  - `gapmap_palace_warmup` → triggers `warmup_model()` (download + extract ONNX) without leaving Claude
  - `gapmap_semantic_search(query, topic?, source_type?, k=10, rerank=True)` → hybrid vector + BM25, identical scoring to the app's Settings → Semantic search
  - `gapmap_related_posts(post_id, k=10, topic?)` → "more like this" via cosine on the post's existing embedding
  - `gapmap_palace_reindex()` → idempotent backfill — safe to interrupt
...
12,108 posts indexed at `~/Library/Application Support/com.shantanu.gapmap/reddit-myind/palace/`, identical to the app's read view.
...
- MCP fetches (`gapmap_fetch_posts` etc.) auto-index into Palace as long as the model is ready in the MCP process. If the user enabled Palace after some MCP-side fetches, run `gapmap_palace_reindex` to backfill — fast (~2K posts/min).
