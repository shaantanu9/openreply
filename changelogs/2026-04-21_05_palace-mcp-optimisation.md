...
1. **Zero MCP coverage** — Claude Code could `reddit_search` (Reddit API), `reddit_query_db` (raw SQL), and `reddit_graph_*` (graph traversal), but couldn't do **semantic search** over the 12,108-post local corpus. The 79 MB ONNX model was a sunk cost the MCP never used.
...
- **5 new MCP tools** (file: `src/reddit_research/mcp/server.py`):
  - `reddit_palace_status` → `{installed, ready, count, archive_bytes, expected_bytes, cache_dir, palace_dir}`
  - `reddit_palace_warmup` → triggers `warmup_model()` (download + extract ONNX) without leaving Claude
  - `reddit_semantic_search(query, topic?, source_type?, k=10, rerank=True)` → hybrid vector + BM25, identical scoring to the app's Settings → Semantic search
  - `reddit_related_posts(post_id, k=10, topic?)` → "more like this" via cosine on the post's existing embedding
  - `reddit_palace_reindex()` → idempotent backfill — safe to interrupt
...
- MCP fetches (`reddit_fetch_posts` etc.) auto-index into Palace as long as the model is ready in the MCP process. If the user enabled Palace after some MCP-side fetches, run `reddit_palace_reindex` to backfill — fast (~2K posts/min).
