...
- CLI: `openreply agent link-persona|unlink-persona|personas` (`cli/agent_cmds.py`).
- MCP: `openreply_agent_link_persona|unlink_persona|personas` (`mcp/tools/persona_tools.py`).
...
- `src/openreply/persona/retrieve.py` — shared persona memory retrieval primitives.
- `src/openreply/reply/knowledge.py` — the agent↔persona knowledge blend core.
- `tests/test_reply_knowledge_blend.py` — blend unit tests.
- `docs/agent-persona-blend-flow.md` — canonical flow + design reference.

## Files Modified

- `src/openreply/reply/agent.py` — `agent_personas` table + link CRUD.
- `src/openreply/reply/content.py` — blend into `generate_content`.
- `src/openreply/reply/generate.py` — blend into `generate_reply`.
- `src/openreply/persona/chat.py` — re-import retrieval from `persona/retrieve.py`.
- `src/openreply/persona/graph.py` — new `neighbors()` 1-hop expansion.
- `src/openreply/persona/__init__.py` — export `neighbors`.
- `src/openreply/cli/agent_cmds.py` — link/unlink/personas commands.
- `src/openreply/mcp/tools/persona_tools.py` — agent↔persona link MCP tools.
