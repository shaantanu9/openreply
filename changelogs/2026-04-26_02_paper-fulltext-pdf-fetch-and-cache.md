...
- **`src/reddit_research/mcp/server.py`** — two new MCP tools:
  - `reddit_paper_fulltext(post_id, force, max_chars)` — same shape as `get_full_text`, with `max_chars` truncation so a 200k-char paper doesn't blow MCP message limits on small clients.
  - `reddit_paper_fulltext_status(topic)` — aggregate counts.
  Both tools auto-register the per-call logging events introduced in `2026-04-26_01` (you'll see them in `mcp logs --tool reddit_paper_fulltext`).
...
- `src/reddit_research/mcp/server.py` — `reddit_paper_fulltext` + `_status` tools
...
- Both MCP tools (`reddit_paper_fulltext`, `reddit_paper_fulltext_status`) appear in `_list_tools()`.
