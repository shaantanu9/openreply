...
tool itself needs to call an LLM (e.g. `reddit_synthesize_insights`),
and those calls are persisted with their provider/model so the user
can see what ran. Also loosened `reddit_query_db` to accept read-only
...
- `reddit_query_db` now accepts read-only PRAGMAs (`table_info`,
...
- `reddit_discover_subs` now persists the ranked subs list to
  `mcp_analyses` (kind=`subreddit_ranking`).
- `reddit_analyze_paper` mirrors its analysis (summary/relevance/takeaway)
  into `mcp_analyses` (kind=`paper_analysis`).
- `reddit_analyze_papers_bulk` writes a single rollup row
  (kind=`conclusion`) so the GUI list stays readable.
- **New MCP tool** `reddit_synthesize_insights` — exposes
  `research.insights.synthesize_insights()`. Writes to `mcp_analyses`
  (kind=`insights`).
- **New MCP tool** `reddit_find_gaps` — exposes `research.gaps.find_gaps()`.
  Writes the 4-part report (painpoints / feature_wishes /
  product_complaints / diy_workarounds) to `mcp_analyses` (kind=`gaps`).
- **New MCP tool** `reddit_mcp_analyses_list` — read tool a client LLM
...
- `src/reddit_research/mcp/server.py` — loosened `reddit_query_db`
  guard for read-only PRAGMAs; wired persistence into
  `reddit_discover_subs`, `reddit_analyze_paper`,
  `reddit_analyze_papers_bulk`; added
  `reddit_synthesize_insights`, `reddit_find_gaps`,
  `reddit_mcp_analyses_list`; updated schema doc in `reddit_query_db`
