...
tool itself needs to call an LLM (e.g. `gapmap_synthesize_insights`),
and those calls are persisted with their provider/model so the user
can see what ran. Also loosened `gapmap_query_db` to accept read-only
...
- `gapmap_query_db` now accepts read-only PRAGMAs (`table_info`,
...
- `gapmap_discover_subs` now persists the ranked subs list to
  `mcp_analyses` (kind=`subreddit_ranking`).
- `gapmap_analyze_paper` mirrors its analysis (summary/relevance/takeaway)
  into `mcp_analyses` (kind=`paper_analysis`).
- `gapmap_analyze_papers_bulk` writes a single rollup row
  (kind=`conclusion`) so the GUI list stays readable.
- **New MCP tool** `gapmap_synthesize_insights` — exposes
  `research.insights.synthesize_insights()`. Writes to `mcp_analyses`
  (kind=`insights`).
- **New MCP tool** `gapmap_find_gaps` — exposes `research.gaps.find_gaps()`.
  Writes the 4-part report (painpoints / feature_wishes /
  product_complaints / diy_workarounds) to `mcp_analyses` (kind=`gaps`).
- **New MCP tool** `gapmap_mcp_analyses_list` — read tool a client LLM
...
- `src/reddit_research/mcp/server.py` — loosened `gapmap_query_db`
  guard for read-only PRAGMAs; wired persistence into
  `gapmap_discover_subs`, `gapmap_analyze_paper`,
  `gapmap_analyze_papers_bulk`; added
  `gapmap_synthesize_insights`, `gapmap_find_gaps`,
  `gapmap_mcp_analyses_list`; updated schema doc in `gapmap_query_db`
