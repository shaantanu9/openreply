...
1. `openreply_synthesize_insights` — output validation error: the LLM
   sometimes returned a shape that didn't match the declared schema.
2. `openreply_semantic_search` — "Failed to apply logs to the hnsw segment
...
3. `openreply_search_all` (normal) — 0 hits for natural-language phrases
   because LIKE only matches verbatim substrings.
4. `openreply_search_all` (aggressive) — connection closed when the LLM
   query-expansion or the corrupt palace search hung past the MCP
   transport idle window.
5. `openreply_global_competitors` — schema mismatch (return type was
   declared `list[dict]` but the implementation returned a `dict`).
6. `openreply_query_db` — knock-on `database is locked` / `disk I/O error`
   after the HNSW writer crashed and left the SQLite WAL in a bad state.

## Changes

- **`openreply_synthesize_insights`** wraps the call in try/except and
...
- **`openreply_global_competitors`** signature changed from `list[dict]`
...
- **`openreply_palace_repair`** — new MCP tool exposing the heal +
...
- **`openreply_query_db`** retries once on `database is locked` /
...
- `src/reddit_research/mcp/server.py`
  - `openreply_synthesize_insights` — try/except + response normalisation
  - `openreply_global_competitors` — return type fixed to `dict`, robust
    wrapper handles list / dict / exception cases
  - `openreply_query_db` — retry on transient lock + cache-clear on hard
    failure
  - `openreply_palace_repair` — new tool
