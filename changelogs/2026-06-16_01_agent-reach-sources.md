...
Ported Agent Reach's portable platform readers into native OpenReply sources,
...
- **10 new sources** (`src/openreply/sources/`):
...
- **Wiring**: registered in `sources/__init__.py`, `collect_adapter.SOURCES`,
  `source_families.REDDIT_FAMILY` (reddit_free), MCP tools (`openreply_fetch_*`,
  `openreply_read_*`, `openreply_creds_list/verify`), and CLI (`openreply creds …`).
...
- `src/openreply/sources/{v2ex,web_reader,bilibili,xiaoyuzhou,exa_search,xueqiu,xiaohongshu,linkedin,reddit_free}.py`
- `src/openreply/fetch/_reddit_tiers.py`
...
- `src/openreply/core/public_client.py` — `_proxy()` + proxy-aware httpx clients.
- `src/openreply/sources/_cookie_extract.py` — `COOKIE_REGISTRY` + `extract_cookies()`.
- `src/openreply/sources/{__init__,collect_adapter,source_families,x_twitter}.py` — registration + twitter cookie fallback.
- `src/openreply/fetch/{posts,search}.py` — tiered cascade.
- `src/openreply/mcp/server.py` — fetch/read/creds MCP tools.
- `src/openreply/cli/main.py` — `creds` subcommands.
