...
For every scored gap, Gap Map now lists the actual people voicing the pain — author, the post permalink to reply to, their engagement, the gaps they voiced, and their audience-persona cluster. Turns an insight into outreach (WorthBuild-style). Built from the evidence posts already linked to each gap by the pain-score pass, deduped by author, and cached in a new `gap_evidence_users` table. Verified on "calari tracking app": 12 people across 11 gaps, ranked by engagement.
...
- CLI: `gapmap research gap-audience --topic … [--build] [--gap-id] [--limit] [--csv]`.
- MCP: `gapmap_gap_audience(topic, gap_id, build, limit)` tool.
...
- `src/gapmap/research/gap_audience.py`
