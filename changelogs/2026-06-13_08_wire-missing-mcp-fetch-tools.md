...
was 11 sources collectable in bulk but missing a standalone per-source MCP `openreply_fetch_*`
...
- Added 11 `@mcp.tool() openreply_fetch_*` wrappers in `mcp/server.py` (after
  `openreply_fetch_acled`): `polymarket, truthsocial, digg, tiktok, instagram, threads,
...
- Verified server imports clean and all 11 tools register (51 `openreply_fetch_*` total).
...
- Corrected `docs/specs/PAPER_RESEARCH_ARCHITECTURE.md`: collector convention is
  ...
  status section.
- Flagged a pre-existing duplicate `openreply_paper_citations` definition
...
- `src/openreply/mcp/server.py` — added 11 per-source MCP fetch tools.
