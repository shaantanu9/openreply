...
- Confirmed the user was running the production `/Applications/OpenReply.app` (Developer ID signed + notarized), so source edits required a full signed release to take effect there.
...
- `~/.config/openreply/.env` and `~/.config/reddit-myind/.env`: set `LLM_MODEL=meta/llama-3.1-8b-instruct` (was empty → defaulted to the 70B). Backups saved as `.env.bak.<ts>`. Takes effect on the next chat send (each chat sidecar reloads `.env`).
...
- Swapped the new build into `/Applications/OpenReply.app`; backed up the prior app to `/Applications/OpenReply.app.bak-20260601-152704`; relaunched.

## Notes

- The release bundled the current working tree, which included unrelated in-progress changes beyond the two chat fixes (`api.js`, `collect.js`, `cli.rs`, `openreply-cli.spec`, `pyinstaller-entrypoint.py`, `install.py`, `mcp_bootstrap.js`, `settings.js`).
...
- Modified config: `~/.config/openreply/.env`, `~/.config/reddit-myind/.env`
- Installed: `/Applications/OpenReply.app` (new v0.1.7); backup `/Applications/OpenReply.app.bak-20260601-152704`
