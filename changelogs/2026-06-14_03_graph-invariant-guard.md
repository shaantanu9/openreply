...
- MCP tool `openreply_graph_invariants(topic)` added to server.py, registered in `_TOOL_REGISTRY`
- 7 passing tests across 3 test files (invariants unit, build wiring, MCP surface)

## Files Created

- `src/openreply/graph/invariants.py`
...
- `src/openreply/graph/build.py` — added invariant guard call after build_complete record_check
- `src/openreply/cli/main.py` — added `graph-invariants` command to research_app
- `src/openreply/mcp/server.py` — added `openreply_graph_invariants` tool before `openreply_diagnostics`
