...
Added two new MCP tools to the FastMCP server that complete the paper research pipeline. `openreply_paper_research_pipeline` is the orchestrating tool that chains all the existing paper tools (search → rank → fulltext → analyze → store) into a single MCP call. `openreply_papers_for_topic` is a fast read-only companion that returns all analyzed papers for a topic from cache. Together these make the MCP server a first-class academic research surface.

## Changes

- Added `openreply_paper_research_pipeline` to `src/reddit_research/mcp/server.py`
...
- Added `openreply_papers_for_topic` to `src/reddit_research/mcp/server.py`
