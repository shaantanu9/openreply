...
- `main.js` listens for `gapmap:changed` kind='collect' and fires `api.startExtractionWorker().catch(() => {})` — idempotent on the Rust side so rapid collect completions don't stack.
