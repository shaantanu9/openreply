...
- **`src/gapmap/cli/main.py` (`cmd_research_semantic_search`):** bound the
  ...
  (`GAPMAP_PALACE_SEARCH_TIMEOUT`) — clears a cold ONNX one-shot reload (~12-15s)
...
- Forced timeout (`GAPMAP_PALACE_SEARCH_TIMEOUT=0.001`) → returns `busy`
  immediately (no hang).
