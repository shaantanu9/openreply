...
- Slow tools — `reddit_fetch_appstore` averages 19 s and maxes at 37 s, exceeding Claude Code's ~25-30 s tool-call timeout. Even with a stable connection these can cause client-side cancellation. Candidates for chunked/streaming response, page-cap, or pre-warmed cache.
