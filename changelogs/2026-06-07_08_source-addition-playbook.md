...
traced OpenReply's own source-adding wiring (`sources/<name>.py` → `__init__.py` →
...
documents how to add ANY external source to OpenReply, a reusable fetcher template, the
...
- Documented OpenReply's canonical 6-7 file "add a source" recipe with a copy-paste template.
- Compared miroclaw (class/DataResult/router/collector) vs OpenReply (fetch_*/posts-row/collect_adapter) architectures.
