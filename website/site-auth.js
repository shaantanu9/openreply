...
(function () {
  ...
  async function startTrial() {
    ...
    const session = await getSession();
    if (!session?.access_token) {
      throw new Error("Sign in required to start a trial.");
    }
    const res = await fetch(`${licenseApiBase}/api/v1/trial/start`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${session.access_token}`,
    ...
    let body;
    try { body = JSON.parse(raw); } catch { body = { error: raw }; }
    if (!res.ok) throw new Error(body?.error || `Trial start failed (${res.status})`);
    return body;
  }
