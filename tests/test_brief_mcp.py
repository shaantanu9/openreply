"""Task 4 — MCP tools for clarified-brief: openreply_brief_get / openreply_brief_set.

Mirrors tests/test_mcp_provenance_tools.py: seeds the DB via brief helpers,
reloads server.py so it inherits the patched OPENREPLY_DATA_DIR, then calls the
...
def test_brief_mcp_get_set_roundtrip(monkeypatch, tmp_path):
    ...
    brief_set = registry["openreply_brief_set"]
    brief_get = registry["openreply_brief_get"]
