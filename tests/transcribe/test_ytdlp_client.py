...
def test_is_newer_semantic_compare():
    from reddit_research.transcribe.ytdlp_client import _is_newer
...
def test_overlay_dir_uses_data_root_env(tmp_path, monkeypatch):
    ...
    from reddit_research.transcribe.ytdlp_client import overlay_dir
...
def test_cooldown_skips_update(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("REDDIT_MYIND_DATA_DIR", str(tmp_path))
    from reddit_research.transcribe.ytdlp_client import (
        ensure_latest_ytdlp,
        overlay_dir,
    )
    od = overlay_dir()
...
def test_force_bypasses_cooldown_but_handles_pypi_down(tmp_path, monkeypatch):
    monkeypatch.setenv("REDDIT_MYIND_DATA_DIR", str(tmp_path))
    from reddit_research.transcribe import ytdlp_client as mod
