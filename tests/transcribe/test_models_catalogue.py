...
def test_catalogue_has_every_expected_tier():
    from openreply.transcribe.models import MODELS, DEFAULT_TIER
...
def test_default_tier_falls_back_to_default_when_nothing_installed(tmp_path, monkeypatch):
    ...
    # list_installed() probes HF cache + OPENREPLY_WHISPER_MODELS_DIR too.
    ...
    from openreply.transcribe.models import DEFAULT_TIER, default_tier
...
def test_default_tier_reads_marker_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HF_HUB_CACHE", str(tmp_path / "_hf_empty"))
    monkeypatch.delenv("OPENREPLY_WHISPER_MODELS_DIR", raising=False)
    from openreply.transcribe.models import (
        default_tier,
        models_root,
        set_default_tier,
    )
...
def test_catalogue_marks_installed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HF_HUB_CACHE", str(tmp_path / "_hf_empty"))
    monkeypatch.delenv("OPENREPLY_WHISPER_MODELS_DIR", raising=False)
    from openreply.transcribe.models import catalogue, models_root
