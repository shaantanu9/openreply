...
def test_resolve_topic_returns_input_unchanged_without_alias():
    tr = _try_import("reddit_research.research.topic_resolver")
    if tr is None:
...
def test_soft_delete_hides_then_restore_unhides():
    trash = _try_import("reddit_research.research.trash")
    if trash is None:
...
def test_quality_gate_heuristics(row, strict, expect):
    qg = _try_import("reddit_research.research.quality_gate")
    if qg is None:
...
def test_feedback_record_and_prompt_integration():
    fb = _try_import("reddit_research.research.feedback")
    if fb is None:
...
def test_saved_views_apply_filter():
    sv = _try_import("reddit_research.research.saved_views")
    if sv is None:
...
def test_prompt_override_roundtrip():
    ps = _try_import("reddit_research.research.prompt_store")
    if ps is None:
