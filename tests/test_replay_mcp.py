"""Task 2G.2 — MCP tools openreply_runs_list and openreply_run_get.
...
def test_openreply_runs_list_returns_run(monkeypatch):
    ...
    fn = server_mod._TOOL_REGISTRY["openreply_runs_list"]
...
def test_openreply_runs_list_empty_topic_returns_all(monkeypatch):
    monkeypatch.setenv("OPENREPLY_DATA_DIR", tempfile.mkdtemp())
...
def test_openreply_run_get_returns_checks_and_lineage(monkeypatch):
    monkeypatch.setenv("OPENREPLY_DATA_DIR", tempfile.mkdtemp())
    ...
    fn = server_mod._TOOL_REGISTRY["openreply_run_get"]
    result = fn(run_id="RX")
...
def test_openreply_run_get_unknown_returns_empty(monkeypatch):
    monkeypatch.setenv("OPENREPLY_DATA_DIR", tempfile.mkdtemp())
