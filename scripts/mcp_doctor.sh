...
# for the openreply server across all 3 MCP clients (Claude Code, Claude
...
e = (d.get("mcpServers") or {}).get("openreply")
if not e:
  print("  ℹ no openreply entry"); sys.exit(0)
...
if "uv" in cmd:
  ...
  print(f"     consider: command={os.path.expanduser('~/Documents/GitHub/reddit-myind/.venv/bin/openreply')}, args=['mcp','serve']")
...
if expected_tag and tag == expected_tag:
  ...
  print(f"  ❌ MCP_CLIENT_TAG missing — this client shares a pidfile with the others. Cross-client SIGTERM will disconnect mid-tool-call. Re-run: openreply mcp install --client {expected_tag}")
...
dd = env.get("OPENREPLY_DATA_DIR") or ""
if dd: print(f"  ✅ data_dir env: {dd}")
else: print("  ⚠  OPENREPLY_DATA_DIR unset — server uses fallback path, may not see your DB")

tok = env.get("OPENREPLY_TOKEN") or ""
...
for log in \
  ...
  "$HOME/Library/Logs/Claude/mcp-server-openreply.log" \
...
if [ -x "$PROJECT_DIR/.venv/bin/openreply" ]; then
  CMD="$PROJECT_DIR/.venv/bin/openreply"
  ...
  ( OPENREPLY_DATA_DIR="$SMOKE_DD" MCP_TAKEOVER_STALE_LOCK=1 \
...
"
  ...
  warn "no $PROJECT_DIR/.venv/bin/openreply — run 'uv sync' or 'pip install -e .' in $PROJECT_DIR"
fi

echo ""
info "Next steps if any ❌ above:"
info "  1. To rewrite all 3 client configs to use the venv binary directly (recommended):"
info "       $PROJECT_DIR/.venv/bin/openreply mcp install --client claude-code"
info "       $PROJECT_DIR/.venv/bin/openreply mcp install --client claude-desktop"
info "       $PROJECT_DIR/.venv/bin/openreply mcp install --client cursor"
