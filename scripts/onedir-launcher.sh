...
# <1 s once warm.
#
# Tauri's externalBin mechanism copies THIS script to Contents/MacOS/openreply-cli,
# so both the streaming path (`app.shell().sidecar("openreply-cli")`) and the
# warm-daemon path (`resolve_bundled_sidecar()` → current_exe dir) keep
# resolving "openreply-cli" unchanged. We just `exec` the real onedir exe — exec
...
if [ -n "${OPENREPLY_ONEDIR_EXE:-}" ] && [ -x "${OPENREPLY_ONEDIR_EXE}" ]; then
  exec "${OPENREPLY_ONEDIR_EXE}" "$@"
fi

# 2) Known candidate layouts relative to Contents/MacOS/.
for c in \
  "$HERE/../Resources/binaries/openreply-cli-onedir/openreply-cli" \
  "$HERE/../Resources/openreply-cli-onedir/openreply-cli" \
  "$HERE/../Resources/_up_/binaries/openreply-cli-onedir/openreply-cli" \
  "$HERE/openreply-cli-onedir/openreply-cli"; do
...
RES="$HERE/../Resources"
if [ -d "$RES" ]; then
  FOUND="$(/usr/bin/find "$RES" -maxdepth 4 -type f -name openreply-cli -perm -111 2>/dev/null | head -1)"
  if [ -n "$FOUND" ] && [ -x "$FOUND" ]; then exec "$FOUND" "$@"; fi
fi

echo "openreply-cli: onedir engine not found near $HERE (looked in ../Resources)" >&2
exit 127
