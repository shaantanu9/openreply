...
" "$tool" "$args_json")
  if ! mcp_call "$sid" "$out" "openreply_jobs_submit" "$args_outer"; then
...
JOB=$(submit_job "$SID" "openreply_does_not_exist" '{}')
if [ -z "$JOB" ]; then
...
JOB1=$(submit_job "$SID" "openreply_palace_status" '{}')
if [ -n "$JOB1" ]; then
...
mcp_call "$SID" /tmp/list.txt "openreply_jobs_list" '{"limit":5}' && \
...
mcp_call "$SID" /tmp/c.txt "openreply_jobs_cancel" '{"job_id":"j_does_not_exist"}'
...
JOB2=$(submit_job "$SID" "openreply_topic_stats" '{}')
...
JOB3=$(submit_job "$SID" "openreply_research_collect" \
...
if [ -z "$JOB3" ]; then
  ...
  # Cancel it
  mcp_call "$SID" /tmp/cancel.txt "openreply_jobs_cancel" "{\"job_id\":\"$JOB3\"}"
...
for i in 1 2 3 4 5; do
  J=$(submit_job "$SID" "openreply_palace_status" '{}')
  JOBS+=("$J")
done
SUBMITTED=${#JOBS[@]}
...
test_wrapper() {
  ...
  mcp_call "$SID" /dev/null "openreply_jobs_cancel" "{\"job_id\":\"$job\"}" >/dev/null 2>&1
  # Tiny pause so cancel lands before the next submission grabs the worker.
  sleep 1
}

test_wrapper "openreply_paper_fulltext" '{"post_id":"smoke_bogus_'$$'"}' "[fulltext]" "wrapper-paper-fulltext" 15
test_wrapper "openreply_palace_warmup" '{}' "[warmup]" "wrapper-palace-warmup" 15
test_wrapper "openreply_analyze_papers_bulk" '{"topic":"smoke_no_papers_'$$'"}' "[paper-bulk]" "wrapper-analyze-papers-bulk" 15
...
test_wrapper "openreply_find_gaps" '{"topic":"smoke_empty_'$$'","corpus_limit":5,"min_score":0}' "[gaps]" "wrapper-find-gaps" 30
test_wrapper "openreply_paper_draft_generate" '{"topic":"ai"}' "[paper-draft]" "wrapper-paper-draft-generate" 30
test_wrapper "openreply_graph_build_relations" '{"topic":"ai"}' "[graph-relations]" "wrapper-graph-build-relations" 30

# Note: openreply_research_collect was already exercised in "Live progress" section.
# Note: openreply_palace_reindex is intentionally not exercised end-to-end —
...
JX=$(submit_job "$SID" "openreply_palace_reindex" '{}')
if [ -n "$JX" ]; then
  sleep 2
  mcp_call "$SID" /dev/null "openreply_jobs_cancel" "{\"job_id\":\"$JX\"}" >/dev/null 2>&1
