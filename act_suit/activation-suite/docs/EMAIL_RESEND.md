...
| `EMAIL_FROM` | Sender, e.g. `OpenReply <keys@openreply.myind.ai>`. Defaults to `onboarding@resend.dev` (Resend's test sender — only delivers to your own account email). |
...
2. **Verify your sending domain** (e.g. `openreply.myind.ai`): Resend → Domains →
...
4. Set env on Vercel (Project → Settings → Environment Variables):
   - `RESEND_API_KEY=re_…`
   - `EMAIL_FROM="OpenReply <keys@openreply.myind.ai>"`
5. **Redeploy.** Done.
...
3. Update env: `EMAIL_FROM="OpenReply <keys@newdomain>"` (and `RESEND_API_KEY` if rotated).
