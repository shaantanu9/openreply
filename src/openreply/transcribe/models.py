...
Models live under ``$OPENREPLY_DATA_DIR/whisper-models/<tier>/`` by default,
but the app also auto-detects Whisper models the user already has on disk
from other Python projects (HuggingFace hub cache, ``OPENREPLY_WHISPER_MODELS_DIR``,
...
Resolution priority for a given tier:
    ...
    2. Env override:     ``$OPENREPLY_WHISPER_MODELS_DIR/<tier>/``
...
def _data_root() -> Path:
    ...
    env = os.environ.get("OPENREPLY_DATA_DIR")
    if env:
        return Path(env)
    return Path.home() / ".config" / "openreply"
...
def _common_dir_candidates(tier: str) -> list[tuple[str, Path]]:
    ...
    env_dir = os.environ.get("OPENREPLY_WHISPER_MODELS_DIR")
    if env_dir:
