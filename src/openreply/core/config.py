...
Resolution order (first non-empty wins):
  1. OPENREPLY_DATA_DIR env var (set by the Tauri sidecar)
  2. Platform app-data dir:
       macOS:   ~/Library/Application Support/com.shantanu.openreply/openreply
       Linux:   ~/.local/share/com.shantanu.openreply/openreply
                (or $XDG_DATA_HOME if set)
       Windows: %APPDATA%\\com.shantanu.openreply\\openreply
  3. ~/.config/openreply/data (legacy fallback; old `reddit-myind` migrated on first run)
...
_USER_CONFIG_DIR = Path.home() / ".config" / "openreply"
...
_TAURI_BUNDLE_ID = "com.shantanu.openreply"
_APP_SUBDIR = "openreply"
_LEGACY_APP_SUBDIR = "reddit-myind"
_LEGACY_DB_FILENAME = "reddit.db"
_DB_FILENAME = "openreply.db"


def _migrate_legacy_layout(target: Path) -> None:
    """One-shot migration from the old `reddit-myind` layout.

    Renames `<bundle>/reddit-myind` → `<bundle>/openreply` on disk, and
    renames `reddit.db*` → `openreply.db*` inside. Runs only when:
...
def _canonical_app_data_dir() -> Path:
    """Platform-standard app-data folder for OpenReply.
...
def _resolve_data_dir() -> Path:
    ...
    env_override = os.getenv("OPENREPLY_DATA_DIR")
    if env_override:
    ...
    # freshly-created empty `openreply` folder.
    ...
    warnings.warn(
        ...
        "Set OPENREPLY_DATA_DIR to pin to a canonical location.",
        stacklevel=2,
    )
    return fallback
...
class Config:
    ...
    def db_path(self) -> Path:
        return self.data_dir / "openreply.db"
    ...
    def mode(self) -> str:
        ...
        Forced via OPENREPLY_MODE if set; otherwise auto-selected.
        """
        forced = os.getenv("OPENREPLY_MODE", "").strip().lower()
    ...
    def require_reddit(self) -> None:
        ...
        if missing:
            raise RuntimeError(
                ...
                "(read-only). For user actions, also run `openreply auth login`."
            )
...
def load_config() -> Config:
    ...
    return Config(
        ...
        reddit_user_agent=os.getenv("REDDIT_USER_AGENT") or "openreply/0.1",
