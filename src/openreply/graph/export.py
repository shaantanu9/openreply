"""Graph exporters — JSON + self-contained HTML "OpenReply" viewer.
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Per-section "re-run extraction" button (posts openreply:enrich to host app). */
  .sec-refresh {
...
<footer>openreply · opportunity map · <a href="https://github.com/myind-ai/openreply/blob/main/docs/methodology.md" target="_blank">methodology</a></footer>
...
function renderExec() {
  ...
  const markdown = (() => {
    const lines = [`# OpenReply — ${topicName}`, ""];
  ...
  document.getElementById("savePng").addEventListener("click", async e => {
    ...
    try {
      ...
      const slug = (DATA.meta.topic || "openreply-map").replace(/[^a-z0-9]+/gi, "-").toLowerCase();
      const link = document.createElement("a");
      link.download = `openreply-map-${slug}.png`;
      link.href = dataUrl;
      link.click();
...
function _postSelection(payload) {
  try { window.parent.postMessage({ type: "openreply:select", selection: payload }, "*"); } catch (e) { /* sandboxed */ }
}
...
// The chat (parent window) posts {type:'openreply:focus', url, title} when a user
...
window.addEventListener("message", (e) => {
  const d = e.data;
  if (!d || d.type !== "openreply:focus") return;
  try { focusByCitation(d); } catch (err) { /* non-fatal */ }
});

// ── per-section "Re-run extraction" → ask the host app to enrich ───────────
// The leftbar section ↻ buttons post {type:'openreply:enrich', category} to the
...
document.addEventListener("click", (e) => {
  ...
  btn.textContent = "…";
  try { window.parent.postMessage({ type: "openreply:enrich", category: cat }, "*"); } catch (err) { /* sandboxed */ }
...
window.addEventListener("message", (e) => {
  const d = e.data;
  if (!d || d.type !== "openreply:enrich:done") return;
