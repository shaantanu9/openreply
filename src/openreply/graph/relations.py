...
Env tunables:
  ...
  OPENREPLY_SOLVE_THRESHOLD     default 0.50   (potentially_solves / could_address)
  OPENREPLY_REL_MAX_NEIGHBORS   default 8      (cap per-node fanout so UI stays readable)
"""
...
def _build_relates_to(
    ...
    if len(nodes) < 2:
        return 0
    db = get_db()
    solve_threshold = _env_float("OPENREPLY_SOLVE_THRESHOLD", 0.50)
    lexical_floor = _env_float("OPENREPLY_REL_LEXICAL_FLOOR", 0.08)
    bridge_margin = _env_float("OPENREPLY_REL_BRIDGE_MARGIN", 0.08)
...
def build_semantic_relations(topic: str) -> dict[str, Any]:
    ...
    threshold = _env_float("OPENREPLY_REL_THRESHOLD", 0.55)
    max_neighbors = _env_int("OPENREPLY_REL_MAX_NEIGHBORS", 8)
