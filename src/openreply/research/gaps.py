...
def find_gaps(
    ...
    # Disable with OPENREPLY_GAPS_FULLTEXT=0 if a provider's context is too small.
    prefer_fulltext = (_os.getenv("OPENREPLY_GAPS_FULLTEXT") or "1").strip() not in (
        "0", "false", "no",
    )
    ...
    if not rows:
        out["error"] = f"No corpus found for topic={topic!r}. Run `openreply research collect` first."
        return out
