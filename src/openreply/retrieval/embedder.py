...
multilingual paraphrase-MiniLM via the ``OPENREPLY_EMBEDDING_MODEL`` env var.
...
Set once per run:
    ...
    OPENREPLY_EMBEDDING_MODEL=multilingual openreply research collect --topic ...
"""
...
def _resolve_mode() -> str:
    ...
    Honours both the legacy ``OPENREPLY_EMBEDDING_MODEL`` (multilingual /
    default) and the newer ``OPENREPLY_EMBEDDING_BACKEND`` (mlx / onnx /
    multilingual / default). Backend wins when both are set.
    """
    backend = (os.getenv("OPENREPLY_EMBEDDING_BACKEND") or "").strip().lower()
    if backend in ("mlx", "onnx", "multilingual", "default"):
        if backend == "default":
            ...
            return backend
    raw = (os.getenv("OPENREPLY_EMBEDDING_MODEL") or "").strip().lower()
    if raw == "multilingual":
        return "multilingual"
...
def _multilingual_ef():
    ...
    functional even if a user sets ``OPENREPLY_EMBEDDING_MODEL=multilingual``.
    """
    try:
        from chromadb.utils import embedding_functions
