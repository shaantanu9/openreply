...
A PyInstaller **onefile** build (which ``gapmap-cli`` is) extracts the frozen
...
On Gap Map this is not theoretical: leaked dirs accumulated to **93 dirs /
...
def _collect_in_use(candidates: list[str]) -> set[str]:
    ...
    Restricted to ``gapmap`` processes — they are the only ones that hold a
    gapmap ``_MEI`` dir — which keeps the (potentially slow) ``open_files()``
