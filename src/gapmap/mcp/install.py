...
Goals (v1):
  1. Same DB. Set GAPMAP_DATA_DIR in the MCP entry's env so MCP tools
...
def _ensure_mcp_extra_in_project(project_dir: Path) -> dict[str, Any]:
    ...
    Without this, `uv run gapmap mcp serve` raises ModuleNotFoundError on
...
def _resolve_command(bin_path: Path | None, project_dir: Path | None) -> dict[str, Any]:
    ...
    Priority:
      1. Caller passed --bin (PyInstaller binary or `gapmap` on PATH) →
         direct exec, no `uv` middleman. Used by the bundled app on prod.
      2. Caller passed --project-dir → `uv --directory <proj> run gapmap mcp serve`.
         Used in dev where we want the venv-managed environment.
      3. Neither → default to current `sys.executable` + `-m gapmap.cli.main mcp serve`.
    ...
    if project_dir:
        ...
        # PREFERRED: point directly at the project's `.venv/bin/gapmap`
    ...
    return {
        ...
        "args":    ["-m", "gapmap.cli.main", "mcp", "serve"],
    }
...
def status(
    *,
    ...
    out["entry_data_dir"] = env.get("GAPMAP_DATA_DIR")
    out["db_aligned"] = (out["entry_data_dir"] == str(dd))
    out["token_in_env"] = bool(token and env.get("GAPMAP_TOKEN") == token)
...
def install(
    *,
    ...
    # Dev mode: ensure the [mcp] extra is installed so `uv run gapmap mcp
