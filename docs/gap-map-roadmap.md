# Gap Map — Performance + Feature Roadmap
...
- [x] **DB-mtime freshness poll** (2026-04-19) — new `db_mtime` Rust command (cheap stat syscall, no Python spawn) + JS poller in `api.js` that checks every 5 s while document visible. If the SQLite file changed externally (background collect / MCP server / manual CLI), clears the cache and fires a `gapmap:db-changed` window event.
- [x] **Visibility-aware live refresh** (2026-04-19) — Dashboard listens for `gapmap:db-changed` + runs a 30 s background interval (only while visible) to re-fetch every slot; Activity screen also listens so external writes immediately reflect. Both auto-cleanup via `route-gen` + `hashchange`.
