...
- [ ] Currently `openreply_paper_draft_generate`, `openreply_graph_build_relations`,
      and `openreply_paper_fulltext` get start/done beats only — their
...
- [ ] FastMCP supports `ctx.report_progress()` which emits
      ...
      only persists to the row — the agent has to poll `openreply_jobs_get`.
...
- [ ] After any MCP server change, manually:
  ...
  3. In a Cursor chat, ask the agent to call `openreply_jobs_list({})`.
...
2026-05-01 — `openreply_research_collect`, `openreply_palace_reindex`,
`openreply_palace_warmup`, `openreply_analyze_papers_bulk`,
`openreply_find_gaps`.

### ✅ Start/done beats for 3 monolithic long tools
2026-05-01 — `openreply_paper_fulltext`, `openreply_paper_draft_generate`,
