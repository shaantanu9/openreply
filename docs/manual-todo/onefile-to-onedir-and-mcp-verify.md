...
- [ ] **CRITICAL — rebuild to ship the chromadb bundling fix** (`changelogs/2026-06-01_11`).
  The bundled `openreply-cli` is missing `chromadb.telemetry.product.posthog` (dynamic
  ...
  Fix already applied to `openreply-cli.spec` (added `'chromadb'` to the collect_all loop).
  Rebuild `openreply-cli` + reinstall to deliver it. This is independent of (and more urgent
  than) the onedir change below.


- [ ] **Convert `openreply-cli.spec` from onefile → onedir** (the real fix).
  - Add a `COLLECT(exe, a.binaries, a.datas, name='openreply-cli')` after `EXE(...)`.
  - Result: `dist/openreply-cli/openreply-cli` + `dist/openreply-cli/_internal/`. **No runtime
  ...
  - ⚠️ Tauri sidecar packaging change: the app bundles a single binary at
    `OpenReply.app/Contents/MacOS/openreply-cli`. With onedir you must copy the whole
    `openreply-cli/` folder into the app and have Rust spawn the inner binary. Update
...
- [ ] **Verify the shipped `_MEI` sweep runs in the bundled build.** After the next
  build, launch the bundled `openreply-cli` a few times, kill some mid-run, then confirm
...
- [ ] **Make the MCP card's "Connected" badge call `status(probe=True)`** (e.g. behind a
  ...
  mere config presence. The Python/CLI side is done: `openreply mcp status --probe --json`
...
- [ ] If the installed (onefile) app is used heavily before the onedir build ships,
  ...
  `pkill -9 -f "OpenReply.app/Contents/MacOS/openreply-cli" && rm -rf /var/folders/*/*/T/_MEI*`
