# Gap Map: A Local-First, Multi-Source Research System with a Unified Evidence Contract and Multi-Agent Deliberation

**Working Paper — v0.1**
**Date:** 2026-06-14
**Status:** Working paper (not peer-reviewed). Describes the design and
implementation of the Gap Map system (`reddit-myind`) as of the
...
literature). Gap Map is a local-first desktop system that collects all three
...
Gap Map is built around four beliefs (stated in the project's own vision):
...
academic literature, if at all, as an external link. Gap Map differs by placing
...
relationships. Gap Map's paper subsystem (§6) overlaps here — citation edges,
...
Gap Map uses a local vector index ("palace": a persistent ChromaDB collection
...
answer once. Gap Map's deliberation layer (§7) applies this specifically to
...
device. Gap Map is local-first by construction (Tauri shell + Python sidecar +
...
per-source MCP preview tool (`gapmap_fetch_<name>`), inclusion in the bulk
...
- `gapmap_paper_citations(paper_id, limit)` — *who cites one paper* (forward).
- `gapmap_paper_references(paper_id, limit)` — *what one paper cites* (backward).
- `gapmap_paper_citation_graph(topic, limit)` — *topic-wide*: fetch each
...
(The first and third previously shared the name `gapmap_paper_citations`,
which caused the MCP framework to silently drop one of two same-named tools;
the topic-wide builder was renamed to `gapmap_paper_citation_graph` so both are
...
Gap Map resolves this with a **read/write split**:
...
- **No outcome study.** We have not measured whether Gap Map improves decision
...
- **Outcome evaluation.** A task-based study (find the real gap, support a claim
  with evidence) comparing Gap Map against conventional tools.
...
Gap Map shows that a single design decision — make every source return the
...
| Concern | Location |
|---|---|
| Per-source fetchers (~50) | `src/gapmap/sources/*.py` |
| Shared polite HTTP | `src/gapmap/sources/_http.py` |
| Collector + `SOURCES` dispatch (64) | `src/gapmap/sources/collect_adapter.py` |
| Collect orchestrator (thread pool) | `src/gapmap/research/collect.py` |
| Academic-source source-of-truth | `src/gapmap/research/sources.py` |
| One-call paper pipeline | `src/gapmap/research/paper_pipeline.py` |
| Paper LLM analysis | `src/gapmap/research/paper_analyze.py` |
| Citation graph | `src/gapmap/research/paper_citations.py`, `paper_references.py`, `paper_relations.py` |
| Fleet flow (governed) | `src/gapmap/research/fleet_flow.py` |
| Debate orchestrator | `src/gapmap/research/debate_run.py` |
| Dynamic roles + deliberation | `src/gapmap/research/deliberate.py` |
| MCP tool surface | `src/gapmap/mcp/server.py` |
| CLI surface | `src/gapmap/cli/main.py` |
...
app-tauri/src-tauri/binaries/gapmap-cli-aarch64-apple-darwin health --json
```
