# miroclaw_jyotish → Gap Map — Full Analysis, Reuse Decision, Value Proposition & Data-Source Assessment
...
> **Target repo:** `reddit-myind` (Gap Map) — Tauri 2 + Python (PyInstaller) sidecar
...
1. What is miroclaw, what's reusable, and at what level would porting it bring things into Gap Map?
...
4. Can miroclaw's **12 finance data sources** help Gap Map's research & analysis?
...
miroclaw is **downstream of Gap Map.** Its own `PRODUCT_DEFINITION.md` states its audience/personas are derived from "a **Gap Map corpus**" via `gapmap search`, `gapmap audience_personas`, `gapmap find_gaps`. miroclaw is therefore a **consumer** of this very repo — not something this repo would import. The data flow is `Gap Map → miroclaw`, not the reverse.

### Shared ancestry
Gap Map's `research/deliberate.py` header says *"adapted from autoresearch:predict"* — the **same lineage** as miroclaw's strategy ensemble. The multi-persona-consensus idea was already ported into Gap Map once.

---

## PART B — The three portable units (per miroclaw's own PORTING_GUIDE)

miroclaw's authors documented exactly three independently liftable cores. **All three require you to satisfy a tiny LLM interface (`chat` / `chat_json`) — which Gap Map already has** (`research/strategy_common.run_llm_json` + `core/client.complete`).
...
### P3 — Strategy ensemble *(easiest, but Gap Map already has it)*
...
## PART C — Persona systems compared (miroclaw vs Gap Map)
...
| | miroclaw "persona" | Gap Map "persona" |
|---|---|---|
...
| **Gap Map files** | — | `research/audience.py`, `persona/{store,ingest,chat,graph,conclude,teach,share}.py` |

**Conclusion:** Gap Map's persona systems are **corpus-native and citation-grounded** — arguably richer for research than miroclaw's synthetic-entity personas. The only thing miroclaw adds is the **interacting social simulation** (P2), i.e. "watch a population *react over time*," which Gap Map does not have.
...
## PART E — What Gap Map ALREADY has (the overlap matrix)

| miroclaw unit | In Gap Map today? | Where / note |
|---|---|---|
...
| **Historical/time-travel fetch** | ✅ **Partial** | `gapmap_fetch_historical`, `gapmap_corpus_temporal_split` |
| **P1 prediction engine** (scorer + walk-forward + evolution) | ❌ **No** | Net-new |
| **P2 OASIS social simulation** | ❌ **No** | Net-new |

---

## PART F — Will porting add *all* of miroclaw into Gap Map? (NO)

The plan ports the **transferable machinery only** — re-pointed at Gap Map's domain. The reusable cores are ~20–30% of miroclaw's codebase; the remaining ~70% is finance-specific and would be useless or harmful inside a gap-discovery tool.

| miroclaw piece | Into Gap Map? | Why |
|---|---|---|
...
| **Flask app / frontend / Docker / 4 LLM modes** | ❌ Out | Gap Map has its own shell + provider system |
...
| miroclaw file | Action in Gap Map | Effort |
|---|---|---|
...
| `evolution/loop_runner.py` | **Copy**, wire to Gap Map data dir | Low |
| `evolution/prediction_engine.py` | **Adapt** (set Gap Map's walk-forward windows) | Med |
...
Today Gap Map **describes the present** ("here are the painpoints/gaps that exist now"). P1 makes it **forecast and self-measure**.
...
> *Gap Map stops being a "what's broken now" snapshot and becomes a "what's about to matter" forecaster — with a measured accuracy score so users can actually trust the prioritization.*

---

## PART J — Can miroclaw's 12 finance data sources help Gap Map? (direct answer)
...
| # | miroclaw source | Gap Map status | Verdict for Gap Map |
|---|---|---|---|
| 5 | **Google Trends** | ✅ Already have (`fetch_trends`) | **Redundant** |
| 7 | **Google News** | ✅ Already have (`fetch_gnews`) | **Redundant** |
| 8 | **India RSS** | ✅ Already have (`fetch_rss` + `rss_catalog`, generic) | **Redundant** (Gap Map's is more general) |
| 4 | **GDELT** | ❌ Not present | **Worth adding (MEDIUM)** — structured global news/event coverage with **historical backfill**, which Gap Map's news lacks. Useful for event-driven topics + as a ground-truth signal for the forecast engine. |
| 6/9 | **DuckDuckGo / Tavily** | ❌ Not present (Gap Map has *no* general web search) | **Worth considering (LOW–MED)** — a generic web-search fallback for context/seed-docs and forecast grounding. DDG keyless; Tavily needs a free key. |
| 2/10/11 | **World Bank / FRED / BIS** | ❌ Not present | **Niche (LOW)** — only useful to enrich the **market-sizing / TAM** feature (`gapmap_market_sizing`) with real macro/economic context. Not for painpoint/gap discovery. Optional, key-gated. |
...
1. **GDELT** — the single most defensible *additive* source for Gap Map (historical, structured, event-aware news). Add as a normal `fetch_gdelt` adapter following the existing `sources/` pattern.
2. **A general web-search layer (DuckDuckGo/Tavily)** — fills a real hole: Gap Map currently has no broad web search, only specific platforms. Most valuable as a **seed/ground-truth context source** for the P1 forecast engine.
...
- **3 of 12** are duplicates Gap Map already has.
...
So: **the 12 finance sources do NOT broadly help Gap Map's gap research** (different data philosophy), **except GDELT + a web-search layer**, which are worth adding — primarily to strengthen the forecasting engine's seed-docs and ground truth.

---

## PART K — Data philosophy difference (why most sources don't transfer)

| | miroclaw sources | Gap Map sources |
|---|---|---|
...
| **Transferability** | Low — only where Gap Map needs numeric context (forecast ground truth, market sizing) | — |

This is the root reason "use the 12 sources" mostly doesn't apply: they were built to *measure outcomes for scoring*, which is exactly what the **P1 ground-truth seam** needs — but Gap Map's ground truth for *gap salience* is **its own corpus over time**, not financial tickers.
...
## Appendix 2 — Gap Map's sources (current)
...
- `docs/product/PRODUCT_DEFINITION.md` — audience personas + the Gap Map dependency.
