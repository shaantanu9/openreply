# Gap Map Public Launch Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a v0.1.0 public, OSS-buildable Gap Map release where search → gather → relate → conclude → chat all work end-to-end in a shareable signed DMG, with auto-update.

**Architecture:** 5 phases — Foundation (sidecar + feature-flag license gate + OSS hygiene), Flow (Topic Dashboard + Brief orchestrator), Chat (corpus chat tab using existing `research/chat.py`), Sign+Release (notarized DMG + GitHub Release), Auto-update (tauri-plugin-updater). Each phase is independently shippable. Spec: `docs/superpowers/specs/2026-05-24-gapmap-public-launch-design.md`.
...
app-tauri/vite.config.js                   # P0 — __GAPMAP_LICENSE_GATE_ENABLED__ define
...
fn main() {
    ...
    let secret = match (std::env::var("JWT_DESKTOP_SECRET"), license_gate_enabled, profile.as_str()) {
        ...
        (Err(_), false, _) => {
            "gapmap-oss-public-no-gate-placeholder-secret-32chars".to_string()
        }
...
function isLicenseActivatedLocally() {
  ...
  if (typeof __GAPMAP_LICENSE_GATE_ENABLED__ !== 'undefined' && !__GAPMAP_LICENSE_GATE_ENABLED__) {
    return true;
  }
...
Open `app-tauri/src/screens/welcome.js`. Find the license-key input region (search for `license`, `activation`, or `gapmap.license.activated`). Wrap the rendering of the license-input step with:

```js
const LICENSE_GATE_ENABLED = (typeof __GAPMAP_LICENSE_GATE_ENABLED__ !== 'undefined')
  ? __GAPMAP_LICENSE_GATE_ENABLED__
  : true;
...
cd app-tauri
npm run build
GAPMAP_LICENSE_GATE=true npm run build
```
...
Mirrors the Rust feature flag on the frontend. `__GAPMAP_LICENSE_GATE_ENABLED__`
...
- `vite.config.js` — define `__GAPMAP_LICENSE_GATE_ENABLED__`.
...
git commit -m "$(cat <<'EOF'
feat(license): frontend gate conditional on __GAPMAP_LICENSE_GATE_ENABLED__ (default OFF)
...
[![Release](https://img.shields.io/github/v/release/shaantanu9/gap-map-pro?include_prereleases)](https://github.com/shaantanu9/gap-map-pro/releases)
[![CI](https://github.com/shaantanu9/gap-map-pro/actions/workflows/release.yml/badge.svg)](https://github.com/shaantanu9/gap-map-pro/actions)

**Multi-source product research — desktop app, MCP server, and CLI.**

Gap Map collects signals from 23+ sources (Reddit, Hacker News, arXiv, PubMed,
...
| Surface | Use it when |
|---|---|
| **Desktop app** (`Gap Map.app`) | GUI research — collect, synthesize, graph, chat, export |
...
the [Releases](https://github.com/shaantanu9/gap-map-pro/releases) page.

> **macOS first-launch (ad-hoc-signed builds, pre-v0.1.x notarization):**
> Right-click `Gap Map.app` → Open → Open Anyway. After the first launch,
...
GAPMAP_LICENSE_GATE=true npm run tauri:build:gated
```
```
...
- [ ] Flip GitHub repo to **public**. (Confirm repo name — README references
      `shaantanu9/gap-map-pro` but the local checkout is `reddit-myind`. Pick
...
- [ ] Generate updater signing keypair:
      `cd app-tauri && npm run tauri signer generate -- -w ~/.tauri/gapmap.key`
...
```bash
#!/usr/bin/env bash
# Gap Map sidecar / pipeline smoke test.
...
if [[ "${GATED:-0}" == "1" ]]; then
  export GAPMAP_LICENSE_GATE=true
...
Launch the installed `Gap Map.app`. Pick a topic. Click "Generate brief". Wait for completion. Confirm Brief renders and Workspace strip has clickable cards.

If anything breaks:
- Check `~/Library/Logs/com.shantanu.gapmap/` for stderr.
...
First public Gap Map release. Topic Dashboard with end-to-end pipeline,
...
```bash
cd app-tauri
npx tauri signer generate -w ~/.tauri/gapmap.key
```

The command prompts for a passphrase. The user MUST save:
- The **public key** printed to stdout → goes into `tauri.conf.json`.
- `~/.tauri/gapmap.key` (private) → contents become `TAURI_SIGNING_PRIVATE_KEY` GitHub Secret.
...
"plugins": {
  "updater": {
    "active": true,
    "endpoints": [
      "https://github.com/shaantanu9/gap-map-pro/releases/latest/download/latest.json"
    ],
    "dialog": false,
...
- Quit and re-launch `Gap Map.app`.
...
- Confirm `Gap Map > About` (or About menu) now reads v0.1.1.
