...
> **Status:** Approved for implementation · **Date:** 2026-06-14 · **Target app:** OpenReply (`reddit-myind`, Python + Tauri)
...
Add an **Academic Mode** capability to OpenReply that turns a topic/research
...
constraint runtime), OpenReply **already owns every stage** as a tested Python
...
- Reuse OpenReply's grounding (paper pipeline), deliberation (debate), and
...
- ❌ Deterministic 4-index bibliographic citation-existence verification
  (arXiv/OpenAlex/Crossref/S2) — future sub-project. (Note: OpenReply already
...
|---|---|---|
| Orchestrator | `src/openreply/research/academic_mode.py` :: `run_academic_brief(...)`, `get_academic_brief(topic)` | **NEW** |
...
| MCP | `mcp/server.py` :: `openreply_academic_brief`, `openreply_academic_brief_get` | **NEW (2 tools)** |
...
```
openreply research academic --topic "note apps" \
...
- `openreply_academic_brief(topic, query?, provider?, level?, approved?, rounds?, dynamic_roles?, style?, export_format?)`
- `openreply_academic_brief_get(topic)` — latest brief reader.
...
grounding-before-citing, dependency-ordered gates) in idiomatic OpenReply Python,
