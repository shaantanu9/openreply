# OpenReply — Public Launch + End-to-End Flow Design
...
OpenReply already implements the four headline functions — search (discover),
...
Ship a v0.1.0 public GitHub release of OpenReply where:
...
|---|---|---|
| `search_corpus(query, k=10)` | `openreply_semantic_search` | Find evidence quotes for a question |
| `find_gaps_excerpt(filter)` | `openreply_find_gaps` | Pull specific unmet-needs |
| `top_personas(k=3)` | `openreply_audience_personas_get` | Surface ICPs |
| `cited_quotes(claim)` | `reddit_research_link` | Back a claim with sourced quotes |
| `paper_snippet(query, k=3)` | `openreply_paper_chunk_search` | Pull academic backing |
...
- `build.rs`: drop the unconditional release panic. New logic:
  ...
  let secret = match std::env::var("JWT_DESKTOP_SECRET") {
      ...
      Err(_) if !license_gate_enabled => {
          ...
          "openreply-oss-no-gate-placeholder-secret-32-chars-min".to_string()
      }
...
- `vite.config.js`: inject a build-time constant:
  ...
  ```
- `src/main.js:272-276`:
  - `isLicenseActivatedLocally()` returns `true` when `!__OPENREPLY_LICENSE_GATE_ENABLED__`.
...
- Generate signing keypair:
  `npm run tauri signer generate -- -w ~/.tauri/openreply.key`. Public key
...
- Add an updater endpoint that reads from the GitHub Releases JSON for
  `shaantanu9/openreply` (or whatever the public repo name is).
...
| Sidecar dead | All stages show "sidecar not running" | "Restart OpenReply" toast |
...
- [ ] Flip GitHub repo `shaantanu9/openreply` to public (or current
      name — confirm in `release.yml` target).
- [ ] Upload `openreply_logo.jpg` as the repo's social preview image.
- [ ] Decide on a public name: README says "OpenReply", repo is
      `reddit-myind`. Recommended: rename to `openreply-map` for OSS launch.
