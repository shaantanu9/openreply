...
- **Helpers** (`src/openreply/research/brief.py`): `set_brief(topic, *, goal, constraints, success, audience)` (upsert into topic_prefs, creating the row if absent) · `get_brief(topic) -> dict` · `brief_preamble(topic) -> str` (renders a compact "Research goal / Constraints / Success criteria / Audience" block, or `""` when no brief).
...
- **MCP** (thin): `openreply_brief_get(topic)` / `openreply_brief_set(topic, ...)`.
