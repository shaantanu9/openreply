...
## Components
1. **Backend helper** `traceability_for_artifact(artifact_id) -> list[dict]` in a new `src/gapmap/research/traceability.py` — runs the join above via `get_db().query`, returns source rows (id/title/url/permalink/source_type/author/score). Best-effort: `[]` on error, never raises.
2. **MCP tool** `gapmap_traceability(artifact_id)` in `mcp/server.py` (now clean) — thin wrapper over the helper, mirroring `gapmap_checks_list`/`gapmap_query_db`.
