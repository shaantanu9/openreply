# Gap Map — Dual App Architecture Spec
## Complete Product, Technical & Implementation Guide
### Gap Map Community + Gap Map Pro
...
- Gap Map gets: published content, SEO, network effects,
...
activation, no Gap Map server is involved in any
research operation.
...
- Teams who want to plug Gap Map into their own infrastructure
...
│   │       │   ├── workspace.rs
│   │       │   ├── sweep_result.rs
│   │       │   └── gap_map.rs        ← the ranked output
│   │       └── export/
│   │           ├── markdown.rs
│   │           ├── pdf.rs
│   │           └── csv.rs
│   │
│   ├── gapmap-community/            ← Next.js 14 web app
│   │   ├── package.json
│   │   ├── next.config.ts
│   │   └── src/
...
│   │       └── api/                 ← API routes
│   │
│   └── gapmap-pro/                  ← Tauri 2 desktop app
...
impl GapMapEngine {
    ...
    pub async fn build_gap_map(
        &self,
        workspace_id: &str,
    ) -> Result<GapMap, EngineError>;
...
Deployment:   Vercel
Domain:       gapmap.app
Engine:       gapmap-core compiled to WASM
...
```
GET /explore
├── Featured research (curated by Gap Map team)
...
├── Upvote button
├── "Powered by Gap Map" → download link
...
│      gapmap.app/explore/ai-analytics-gaps-2025           │
...
│      Available on Gap Map Pro ($69) or after upgrade.    │
│                                                          │
│  What gets published:                                    │
...
> Gap Map cannot read them. If you forget your password, your
> API keys are unrecoverable."

---

## 5. Gap Map Pro — full spec
...
Distribution:       Direct download from gapmap.app/pro
...
That is the entire network footprint of Gap Map Pro.
...
```
macOS:    ~/Library/Application Support/com.gapmap.pro/gapmap.db
Windows:  %APPDATA%\com.gapmap.pro\gapmap.db
Linux:    ~/.local/share/com.gapmap.pro/gapmap.db
```
...
Users can point Gap Map Pro at their own database.
...
│      ~/Library/Application Support/gapmap.pro/gapmap.db  │
│      Size: 124 MB                                        │
│                                                          │
│  [○] Your own Postgres                                   │
│      Connection string:                                  │
...
│  [Test connection]                                       │
│                                                          │
│  Note: Gap Map will create its own schema (gapmap_pro.*) │
    ...
    pub async fn from_settings(settings: &Settings) -> Result<Self, StorageError> {
        match settings.db_connection_type.as_deref() {
            Some("postgres") | Some("supabase") => {
                ...
                // Run migrations to create gapmap_pro schema
            ...
            _ => {
                ...
                let db_path = get_app_data_dir()?.join("gapmap.db");
...
│   ├── GapMap.tsx                ← insights visualisation
│   ├── Report.tsx                ← export
│   └── Settings/
...
// Gap Map Pro collects NOTHING by default.
...
//   3. User's own AI provider (BYOK — Gap Map never sees this traffic)
...
{
  "title": "AI Product Analytics — Gap Map",
...
┌─────────────────────────────────────────────────────────┐
│  Share to Gap Map Explore (optional)                     │
│                                                          │
│  You can publish a gap map anonymously to               │
│  gapmap.app/explore without creating an account.        │
│                                                          │
...
pub async fn publish_to_explore(
    ...
    let res = client
        .post("https://gapmap.app/api/v1/pro/publish")
        .bearer_auth(&token)
        .json(&snapshot)
        .send()
        .await
    ...
    Ok(url)  // → "https://gapmap.app/explore/pro-abc123"
}
```
...
# Resend
RESEND_API_KEY=
RESEND_FROM_EMAIL=noreply@gapmap.app

# JWT for Pro cross-publish endpoint
TOKEN_SIGNING_SECRET=        # must match Pro build secret

# App
NEXT_PUBLIC_APP_URL=https://gapmap.app
NEXT_PUBLIC_PRO_DOWNLOAD_URL=https://gapmap.app/pro/download
```
...
ACTIVATION_SERVER_URL=https://gapmap.app
...
The marketing homepage (`gapmap.app`) serves both.
...
app.gapmap.app/register      [How activation works →]
```
...
Prompt:  "Private workspaces require Gap Map Pro"
...
Message: Shown on register if they select "Research" role:
         "Working with sensitive data? Gap Map Pro runs
...
```
9.  Set up gapmap-pro Tauri project
...
16. GapMap visualisation screen
...
```
24. Set up gapmap-community Next.js project
...
```
40. Shared pricing page on gapmap.app
...
gap-map-home.html               Marketing homepage with 3-slide hero,
                                4-tier pricing section, comparison table

gap-map-login.html              Community app login/register/forgot

gap-map-activate.html           Pro app licence management web dashboard
```
